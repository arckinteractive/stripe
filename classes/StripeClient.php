...
class StripeClient {
	...
	public function getSupportedCurrencies() {
		...
		return ($account) ? $account->currencies_supported : [];
	}
	 ...
	 * @return \Stripe\Customer|false
	 */
	public function createCustomer($user = null, $data = []) {
	...
	public function createCard($token, $default = true) {
		...
		try {
			...
			$card = $this->getCustomer()->sources->create(["source" => $token]);
	...
	public function cancelSubscription($id, $at_period_end = false) {
		...
		try {
			...
			return $this->getCustomer()->subscriptions->retrieve($id)->cancel(['at_period_end' => $at_period_end]);
	...
	public function getUpcomingInvoice($subscription_id = '') {
		...
		try {
			...
			return \Stripe\Invoice::upcoming(array_filter($data, function ($e) {
				return isset($e);
			}));
	 ...
	 * @return \Stripe\InvoiceItem|false
	 */
	public function updateInvoiceItem($invoiceitem_id = '', $data = []) {
		try {
			$fields = [
				'amount',
				'description',
	...
	public function getInvoiceItems($invoice_id = null, $limit = 10, $ending_before = null, $starting_after = null, $created = null) {
		...
		try {
			...
			if ($invoice_id) {
				...
				return $invoice->lines->all($params);
				...
				return \Stripe\InvoiceItem::all($params);
			}
	...
	protected function log($exception) {
		if ($exception instanceof \Stripe\InvalidRequestError) {
			$error = elgg_echo('stripe:invalid_request_error', [$exception->getMessage()]);
			elgg_log($exception->getMessage(), 'ERROR');
		} else if ($exception instanceof \Stripe\AuthenticationError) {
			$error = elgg_echo('stripe:authentication_error', [$exception->getMessage()]);
			elgg_log($exception->getMessage(), 'ERROR');
		} else if ($exception instanceof \Stripe\ApiConnectionError) {
			$error = elgg_echo('stripe:api_error', [$exception->getMessage()]);
			elgg_log($exception->getMessage(), 'ERROR');
		} else if ($exception instanceof \Stripe\Error) {
			$error = elgg_echo('stripe:generic_error', [$exception->getMessage()]);
			elgg_log($exception->getMessage(), 'ERROR');
